...
func (r *ClientRepo) Upsert(ctx context.Context, appKey, plainSecret, siteCode, remark string, status int) error {
	...
	data := g.Map{
		"app_secret": hash, "secret_hashed": 1, "site_code": siteCode,
		"status": status, "remark": remark, "updated_at": gtime.Now(),
	}
	if exist.IsEmpty() {
		data["app_key"] = appKey
...
func (r *ClientRepo) FindActive(ctx context.Context, appKey string) (*Client, error) {
	row, err := g.DB().Model("paas_client").Ctx(ctx).Where("app_key", appKey).Where("status", 1).One()
	if err != nil {
		return nil, err
	}
	if row.IsEmpty() {
...
func mapClient(m g.Map) Client {
	return Client{
		Id: g.NewVar(m["id"]).Int64(), AppKey: g.NewVar(m["app_key"]).String(),
		AppSecret: g.NewVar(m["app_secret"]).String(), SecretHashed: g.NewVar(m["secret_hashed"]).Int(),
		SiteCode: g.NewVar(m["site_code"]).String(), Status: g.NewVar(m["status"]).Int(),
		Remark: g.NewVar(m["remark"]).String(), CreatedAt: g.NewVar(m["created_at"]).String(),
